...
function mergeObjects(objects) {
  return objects.reduce((acc, obj) => {
    Object.entries(obj).forEach(([key, value]) => {
      ...
      acc[key] = hasKey ? acc[key] + value : value;
    });
    return acc;
  }, {});
}
...
function compareObjects(obj1, obj2) {
  const firsKeys = Object.keys(obj1);
  const secKeys = Object.keys(obj2);
  if (firsKeys.length !== secKeys.length) return false;

  return firsKeys.every(
    (key) =>
...
function makeWord(lettersObject) {
  const letters = Object.entries(lettersObject).flatMap(([letter, positions]) =>
    positions.map((position) => [position, letter])
  );

  letters.sort((a, b) => a[0] - b[0]);
  return letters.map((pair) => pair[1]).join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    if (bill === 25) {
      ...
      // Use else if instead of a nested if within an else
